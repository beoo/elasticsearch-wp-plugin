...
class EP_API
{
    ...
    public function search($args, $scope = 'current')
    {
        ...
        if (!is_wp_error($request)) {
            ...
            return apply_filters('ep_search_results_array',
                array('found_posts' => $response['hits']['total'], 'posts' => $posts), $response);
        }
    ...
    public function create_network_alias($indexes)
    {
        ...
        $request = ep_remote_request($path,
            apply_filters('ep_create_network_alias_request_args', $request_args, $args, $indexes));
    ...
    public function put_mapping()
    {
        ...
        if (!empty($wp_filter['ep_default_index_number_of_shards'])) {
            ...
            $mapping['settings']['index']['number_of_shards'] = (int)apply_filters('ep_default_index_number_of_shards',
                5); // Default within Elasticsearch
        }
        ...
        if (!empty($wp_filter['ep_default_index_number_of_replicas'])) {
            ...
            $mapping['settings']['index']['number_of_replicas'] = (int)apply_filters('ep_default_index_number_of_replicas',
                1);
        }
    ...
    public function format_args($args)
    {
        ...
        $context = new \Context\StandardContext(implode(',', $user->searchcontext));
        ...
        foreach ($context->rawSearchTerms as $entry) {
            switch ($entry['relation']) {
                case 'OR':
                    foreach ($entry['terms'] as $term) {
                        $squery['bool']['should'][] = array(
                            'multi_match' => array(
                                'query' => $term,
                                'fuzziness' => 6,
                                'type' => 'phrase',
...
//		}
    ...
    public function remote_request($path, $args = array())
    {
        ...
        if (!is_wp_error($host)) { // probably only reachable in testing but just to be safe
            $request = wp_remote_request(esc_url(trailingslashit($host) . $path),
                $args); //try the existing host to avoid unnecessary calls
        }
